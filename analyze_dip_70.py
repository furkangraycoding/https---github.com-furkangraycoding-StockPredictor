...
def analyze_dip_performance_70():
    ...
    # 2. "SAF DİP SİNYALİ" Mantığını Uygula (multiplier: 1.25, prob: 0.70)
    df_all['dip_gap'] = (df_all['AI_Dip_Prob'] - df_all['AI_Peak_Prob']) * 100
    df_all['dip_threshold'] = df_all['RSI'] * 1.25
    df_all['is_pure_dip'] = (df_all['AI_Dip_Prob'] >= 0.70) & (df_all['dip_gap'] > df_all['dip_threshold'])
    ...
    print("\n" + "="*55)
    print("💎 SAF DİP SİNYALİ (%70 Barajı + 1.25 Eşik) PERFORMANSI")
    print("="*55)
    ...
    print("-" * 55)
    print("\n🔍 ÖZET YORUM:")
    print(f"Olasılık barajını %70'e çekmek isabet oranını nasıl etkiledi?")
    print("="*55 + "\n")

if __name__ == "__main__":
    analyze_dip_performance_70()
